...
public class DirVersionReader extends AbstractVersionReader {
    ...
    private Comparator<int[]> comparator = new Comparator<int[]>() {
        ...
        public int compare(final int[] array1, final int[] array2) {
            ...
            for (int i = 0; i < size; i++) {
                final Integer value1 = ArrayUtil.get(array1, i, 0);
                final Integer value2 = ArrayUtil.get(array2, i, 0);
    ...
    protected int[] readVersionArray(final ApplicationContext context) {
        try {
            ...
            final Set<String> versionNumbersIncludingFiles = new HashSet<>(); // 保存包含有文件的版本号
            for (final Resource location : locations) {
                ...
                if (path.endsWith(Strings.SLASH)) { // 相对路径以斜杠结尾的为目录
                    ...
                    if (StringUtil.regexMatch(filename, "\\d+(\\.\\d+)*")) {
                        list.add(MathUtil.parseIntArray(filename, "\\."));
                    }
                } else { // 文件
                    final int index = path.indexOf(Strings.SLASH);
                    if (index > 0) {
                        versionNumbersIncludingFiles.add(path.substring(0, index));
                    }
                }
            }
            Collections.sort(list, this.comparator);
            final int[] array = AlgoLast.visit(list, null);
            final String versionNumber = getVersionNumber(array, array.length);
            if (!versionNumbersIncludingFiles.contains(versionNumber)) { // 版本目录下不包含文件，则需末位减一
