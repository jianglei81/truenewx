        ...
        .extend({
                ...
                minLength : function(validationValue, fieldValue) {
                    ...
                    if (!checker(validationValue, fieldValue)) {
                        ...
                        return [ validationValue,
                                validationValue - (fieldValue.length + enterLength) ];
                    }
                    return undefined;
                },
                number : function(validationValue, fieldValue) {
                    if (validationValue && fieldValue != "") {
                        return typeof (fieldValue) == "number"
                                || this.regExps.number.test(fieldValue);
                    }
            ...
            messages : {
                ...
                "zh_TW" : {
                    required : "{0}不能為空",
                    notBlank : "{0}不能為空或純空格",
                    maxLength : "{0}長度最多可以有{1}位，已超出{2}位",
                    minLength : "{0}長度最少必須有{1}位，還缺少{2}位",
                    number : "{0}必須為數字",
                    integer : "{0}必須為整數",
                    integerLength : "{0}整數位最多可以有{1}位，已超出{2}位",
                    scale : "{0}小數位最多可以有{1}位，已超出{2}位",
                    maxValue : "{0}最大可以为{1}",
                    minValue : "{0}最小可以为{1}",
                    email : "{0}只能包含字母、數字、下劃線、-和.，@兩邊各自的長度應小于64",
                    mobilePhone : "{0}只能是以1开头的11位數字手機號碼",
                    url : "{0}應為格式正確的網址鏈接",
                    regex : "{0}格式錯誤{1}",
                    notContains : "{0}不能包含：{1}",
                    notContainsAngleBracket : "{0}不能包含：< >",
                    notContainsHtmlChars : "{0}不能包含：< > \" '",
                    rejectTags : "{0}不能包含任何標籤",
                    allowedTags : "{0}只能包含標籤：{1}",
                    forbiddenTags : "{0}不能包含標籤：{1}"
                },
                "en" : {
            ...
            getErrorMessage : function(fieldObj, validationName, validationValue) {
                var message = $.tnx.message(this)[validationName];
                if (message) {
            ...
            validateField : function(fieldObj) {
                ...
                var errorMessages = [];
                if (validation) {
                    $.each(validation, function(validationName, validationValue) {
                ...
                var additionalErrorMessage = this.getFieldErrorMessage(fieldObj,
            ...
            appendRequiredTag : function(fieldObj, requriedTag, requiredClass) {
                if (requriedTag) {
                    var appendObj = $("<" + requriedTag + "></" + requriedTag + ">").addClass(
                            requiredClass);
