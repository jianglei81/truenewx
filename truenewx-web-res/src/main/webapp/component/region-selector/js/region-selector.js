    ...
    Region.prototype = {
        ...
        setOptions : function(options) {
            this.options = $.extend({
                ...
                isAutoChange: true,
                subHideByNotGroundSon : false, // 没有孙子级地区时，子级也不显示。 开关(false：不开启，true：开启)
        ...
        initData : function() { // 地区数据初始化
            this.rpc = $.isf.rpc.imports("regionOptionController");
        ...
        render : function() { // 渲染页面效果
            ...
            if (opt.startLevel === 2) {
                if (opt.defaultCountry) {
                    this.initSelectElm(eval("this.data." + opt.defaultCountry
                            + ".subs"), opt.startLevel, false);
                    for (var i = 2; i <= len + 1; i++) {
                        if (this.codes && this.codes[i]) {
                            ...
                            this.initSelectElm(eval("this.data." + str.join(".subs.") + ".subs"), i + 1, false);
                        } else {
            ...
            if(opt.isAutoChange){this.autoChange();}
        },
        initSelectElm : function(result, level, disable) { // 创建国家级下拉选择框
            var _id = $(this.element).attr("id"), selectElm = $("<select>").attr("class", "input-small")
...
//              if (this.options.nullLevel && this.options.nullLevel <= level) {
...
//              }
        ...
        onChange : function() { // 地区联动事件(手动)
            var _id = $(this).nextAll("input").attr("id"), $el = $.data(document, _id),
                val = $(this).val(),
        ...
        autoChange : function() {
            ...
            $(this.element).parent().find("select").each(function() {
                if ($(this).prop("disabled")) {
                    var optionObj = $(this).prev(), val = optionObj.val(),
                        level = optionObj.attr("data-level"), data = $.data(document, _id + "-level" + level);
                    if (val !== "" && val !== null) {
                        ...
                        $("input[name=" + _this.options.regionName + "]").val(_this.getValue());
                    }
                }
            });
        },
        ...
        createOption : function(dom, data, level) {
            var selectElm = $(dom).next().html(""),
                _id = $(dom).nextAll("input").attr("id"),
                nextLelve = selectElm.attr("data-level"),
                opts = new Array();
    ...
    var methods = {
        init : function(option) {
            ...
            $(this).each(function(index, item) {
                ...
                if (!data) {
                    ...
                    result = $.extend({"element" : data.element}, methods);
                    return false;
                }
        ...
        change : function(codes) {
            ...
            var region = $.data(document, $(el).attr("id")),
                codeArry = region.loadDefaultCodes(codes),
                selects = $(el).siblings("select");
            ...
            if(codeArry == null){
                ...
                for(var i = 1; i < selects.length; i++){
                    ...
                    $.data(document, $(el).attr("id") + "-level" + $(selects[i]).attr("data-level"), "");
                }
                return;
            }
